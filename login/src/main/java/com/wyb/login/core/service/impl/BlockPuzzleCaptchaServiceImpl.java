...
import com.wyb.login.core.model.CaptchaRepCodeEnum;
import com.wyb.login.core.model.CaptchaResponse;
import com.wyb.login.core.model.CaptchaTypeEnum;
import com.wyb.login.core.model.vo.CaptchaVO;
import com.wyb.login.core.model.vo.PointVO;
...
import com.wyb.login.core.utils.RandomUtils;
...
public class BlockPuzzleCaptchaServiceImpl extends AbstractCaptchaService {
    ...
    @Override
    public void init(CapthaProperty config) {
        super.init(config);
    }
    ...
    @Override
    public CaptchaResponse get(CaptchaVO captchaVO) {

        // 原生图片
        BufferedImage originalImage = ImageUtils.getOriginal();
        if (null == originalImage) {
            logger.error("滑动底图未初始化成功，请检查路径");
            return CaptchaResponse.errorMsg(CaptchaRepCodeEnum.API_CAPTCHA_BASEMAP_NULL);
        }
        ...
        if (captcha == null || StringUtils.isBlank(captcha.getJigsawImageBase64())
            ...
            return CaptchaResponse.errorMsg(CaptchaRepCodeEnum.API_CAPTCHA_ERROR);
        }
        return CaptchaResponse.successData(captcha);
    }

    @Override
    public CaptchaResponse check(CaptchaVO captchaVO) {
        // 取坐标信息
        String codeKey = String.format(REDIS_CAPTCHA_KEY, captchaVO.getToken());
        if (!CaptchaServiceFactory.getCache(cacheType).exists(codeKey)) {
            return CaptchaResponse.errorMsg(CaptchaRepCodeEnum.API_CAPTCHA_INVALID);
        }
        ...
        try {
            ...
            return CaptchaResponse.errorMsg(e.getMessage());
        }
        if (point.x - Integer.parseInt(slipOffset) > point1.x || point1.x > point.x + Integer.parseInt(slipOffset)
                || point.y != point1.y) {
            return CaptchaResponse.errorMsg(CaptchaRepCodeEnum.API_CAPTCHA_COORDINATE_ERROR);
        }
        // 校验成功，将信息存入缓存
        ...
        return CaptchaResponse.successData(captchaVO);
    }

    @Override
    public CaptchaResponse verification(CaptchaVO captchaVO) {
        if (captchaVO == null) {
